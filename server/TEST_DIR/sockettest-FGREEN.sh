...
function test_send_socket_string
{
	...
	echo ${string} | nc ${target} ${port} -w 1000 > ${new_file}
    cp ${new_file} 'new_file_FGREEN'
	cp ${prev_file} 'prev_file_FGREEN.txt'
	cp ${prev_file} ${expected_file}
	echo ${string} >> ${expected_file}
	cp ${expected_file} 'expected_file_FGREEN'

	#diff ${expected_file} ${new_file} > /dev/null
	diff ${expected_file} ${new_file}
	if [ $? -ne 0 ]; then
        ...
        echo
        echo
		echo ${expected_file} ${new_file}
		diff -y ${expected_file} ${new_file}
